...
class GameScene: SKScene, SKPhysicsContactDelegate ,AVAudioPlayerDelegate {
    ...
    func setupItem() {
        ...
        let upItem = SKAction.moveBy(x: 0, y: 100, duration: 2)
        let downItem = SKAction.moveBy(x: 0, y: -100, duration: 2)
        
        
        let repeatForever = SKAction.repeatForever(SKAction.sequence([upItem,downItem]))
        // アイテムを生成するアクションを作成
        let createItemAnimation = SKAction.run({
            ...
            let centerY = self.frame.size.height/3
            let randomY = CGFloat.random(in: centerY..<centerY+100)
            
            coin.position = CGPoint(x: 100, y:randomY)
            // スプライトに物理演算を設定する
        ...
        let repeatForeverAnimation = SKAction.repeatForever(SKAction.sequence([createItemAnimation, waitAnimation2]))
    ...
    func playSound(name:String){
        guard let path = Bundle.main.path(forResource: "itmeSound", ofType: "mp3") else {
